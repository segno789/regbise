...
class EleventhCorrection extends CI_Controller {
    ...
    */
    ...
    public function EditForms()
    {
        ...
        $RegStdData = array('data'=>$this->EleventhCorrection_model->EditEnrolement($user['Inst_Id']),'grp_cd'=>$user['grp_cd']);
    ...
    public function Applied()
    {
        ...
        $this->load->model('EleventhCorrection_model');
        //  $error['grp_cd'] = $user['grp_cd'];
        $RegStdData = array('data'=>$this->EleventhCorrection_model->EditEnrolement_Applied($user['Inst_Id']),'grp_cd'=>$user['grp_cd']);
        $RegStdData['msg_status'] = $error_msg;
        $userinfo = $Logged_In_Array['logged_in'];
        $this->load->view('common/header.php',$userinfo);
        $this->load->view('common/menu.php',$data);
    ...
    public function Print_correction_Form_Final()
    {
        ...
        foreach ($result as $key=>$data) 
        {
            ...
            $pdf->SetFont('Arial','',10);
            $pdf->SetXY(1.7,0.4);
            $pdf->Cell(0, 0.25, " CORRECTION FORM FOR CLASS ".corr_bank_chall_class1." SESSION 2016-2018", 0.25, "C");
