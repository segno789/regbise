...
class RollNoSlip_model extends CI_Model
{
    ...
    public function get12thStdData($inst_cd)
    {
        ...
        //$query = $this->db->get_where('matric_new..tblbiodata', array('sch_cd' => $inst_cd,'class' => 10, 'iyear' => 2016, 'regpvt'=>1,));
        $inst_cd = '251001';
        $query = $this->db->query("Registration..get12thStdData $inst_cd,2015,12,1,1");
        $rowcount = $query->num_rows();
        if($rowcount > 0)
        {
            return $query->result_array();
    ...
    public function get12thrslipWith_Grp_CD($class,$iyear,$sess,$group_cd,$inst_cd,$sub_cd)
    {
        if($sub_cd == '')
            $sub_cd = -1;
        $query = $this->db->query("Registration..InterSlipInfo_With_Grp_cd $class,$iyear,$sess,$group_cd,$inst_cd,$sub_cd");
        $rowcount = $query->num_rows();
        $row = array();
        $grpwiserow = array();
        if($rowcount > 0)
        {
            ...
            if($sub_cd >0)
            {
                $query1 = $this->db->query("select * from Registration..tbldatesheetinter where rno in( (select rno from Registration..maDatesheet2016  where grp_cd = $group_cd and sub_cd = $sub_cd and sch_cd = $inst_cd))");  
                // $query1 = $this->db->get_where('Registration..maDatesheet2016', array('sch_cd' => $inst_cd,'grp_cd'=>$group_cd,'sub_cd'=>$sub_cd)); 
            }
            else
            {
                $query1 = $this->db->get_where('Registration..tbldatesheetinter', array('sch_cd' => $inst_cd,'grp_cd'=>$group_cd)); 
            }
            $rowcount = $query1->num_rows();
            $row['slip'] = $query1->result_array(); 
            return $row;
