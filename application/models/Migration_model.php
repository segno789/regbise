...
class Migration_model extends CI_Model 
{
     ...
     public function readmission_check($rno)
    {
      ...
      //  $Inst_cd = $User_info_data['Inst_Id'];
        ...
        $where = ' (spl_cd !=  17 OR  spl_cd != 70 OR status !=4)';
     ...
     public function std11thclass_byRollNo($inst_cd)
    {
     ...
     public function tblmainstall11($dist_cd,$gender)
    {

        $query = $this->db->query("select inst_cd,name from admission_online..tblInstitutes_all where edu_lvl in (2,3) and dist_cd<>$dist_cd and isactive=1 and gender=$gender and inst_cd not in (399903,399901,399902) order by inst_cd");    
        $rowcount = $query->num_rows();
        if($rowcount > 0)
        {
            return $query->result_array();
